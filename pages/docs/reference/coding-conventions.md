...
-->除非 `forEach` 的接收者可以为空，或 `forEach` 被用做为长调用链的一部分。<!--
-->在使用多个高阶函数的复杂表达式和循环之间进行选择时，<!--
-->在每种情况下执行的操作，并牢记性能考虑因素。
...
在某些情况下，不带参数的函数可能会与只读属性互换。<!--
-->虽然语义是相似的，但是在某种程度上有一些文体上的约定。<!--
-->基础算法时，首选一个属性：
...
使用扩展功能。每当你有一个主要用于对象的函数时，就考给<!--
-->它一个接受该对象作为接收者的扩展函数。为了尽量减少API污染，限制能见度扩展功能尽可能有意义。根据需要，<!--
-->使用本地扩展功能，成员扩展功能，或具有私密可视性的顶级扩展功能。
...
如果你为一个类声明一个工厂函数，不要给它和类本身同名。<!--
-->更喜欢使用不同的名称明确为什么工厂职能的行为是特殊的。只有当确实没有特殊的语义时，<!--
-->您可以使用与班级相同的名称。<!--
-->例如：
...
如果你有一个对象有多个重载的构造函数，<!--
-->它们不能调用不同的超类构造函数和不能简化为具有默认参数值的单个构造函数，<!--
-->而是更换重载的构造函数工厂功能。

### 平台类型

返回平台类型表达式的公共函数/方法必须显式声明它的<!--
-->Kotlin类型：
